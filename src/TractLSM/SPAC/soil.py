...
def wetness(p, daily_steps, sw0, sw1, sw2, sw3, sw4, sw5, Es, E, Tsoil):
    ...
    # root distri across these layer depths
    depths = [1., 5., 10., 20., 45., 90.]  # cm
    ...
    # absurd runoff but will do for now: max that can get in is 25 mm/hlfhr
    precip = np.minimum(25., 0.8 * precip)
    ...
    sw[:itop + 1] = np.maximum(0., np.minimum(p.theta_sat * vols[:itop + 1],
                                              sw[:itop + 1]))
    ...
    if Tsoil > cst.zero:  # account for snow / frozen soil
        ...
        if np.sum(sw[:itop + 1]) > cst.zero:
            sevap = soil_evap(p, np.sum(sw[:itop + 1]) / p.soil_top_volume)
...
def water_potential(p, sw):
    ...
    When the soil water potential is below the wilting point, we apply
    the dry soil correction from Webb 2000, as parameterised in
    Schneider & Goss 2012.
